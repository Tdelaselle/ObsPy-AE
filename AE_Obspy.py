...
Example:
    python3 AE_Obspy.py -data ./test_2ch/ -save ./ -f mseed -files 0 3 -d 2023-01-01T00:00:00.000 
    -ch 1 2 -sampling 2 5 -sensors nano30 micro200 -head 13 -col 1
...
def load_txt_into_stream(
    ...
    # Get list of files
    filepaths = sorted(glob.glob(dirpath + "*" + "_" + Channel + "_*"))
    ...
    for filepath in tqdm.tqdm(filepaths):
        if ARGUMENTS.columns == 1:
            data.append(np.loadtxt(filepath, skiprows=ARGUMENTS.header_size))  # for 1 column files
        else :  
...
for i in range (len(ARGUMENTS.channels)):
    stream = load_txt_into_stream(ARGUMENTS.datapath,stream,ARGUMENTS.sampling[i]*1e6,str(ARGUMENTS.channels[i]),ARGUMENTS.date,ARGUMENTS.sensors[i])
...
Saving procedures
"""
filepath = ARGUMENTS.savepath+"Stream_"+ARGUMENTS.date
